...
int check_tour(const point *cities, int *tour, int ncities)
{
  ...
  simple_time = compute_time;
  printf("\nTime to find simple tour:  %lld microseconds\n", compute_time);
...
{
  my_tour(cities, tour, ncities); // TODO: define my_tour
}
...
int main(int argc, char *argv[])
{
  ...
  compute_time = (stop_time.tv_sec - start_time.tv_sec) * 1000000L +
                 (stop_time.tv_usec - start_time.tv_usec);
  complex_time = compute_time;
  printf("\nTime to find complex tour: %lld microseconds\n", compute_time);
  ...
  double diff = complex_time - simple_time;
  double differ = diff / 1000000;
  double change = 100 * (diff/simple_time);
